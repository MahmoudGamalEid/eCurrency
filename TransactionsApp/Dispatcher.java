...
public class Dispatcher{
    ...
    class AddTransactionSimpleCmd extends Command implements Runnable {
        ...
        public StringBuffer execute(Connection connection,  Map<String, Object> mapUserData ) throws Exception {
			...
			String              strUserID,
                                strRetailerID;
			int                 amount;
            strUserID    =   (String)mapUserData.get( "userID" );
            strRetailerID=   (String)mapUserData.get( "retailerID" );
            amount=   		 (int)mapUserData.get( "amount" ); 
            
            if( strUserID == null || strUserID.trim( ).length( ) == 0 || 
                strRetailerID == null || strRetailerID.trim( ).length( ) == 0  || 
                amount ==0 )
               return null;
            
            sqlProc = connection.prepareCall("{?=call addTransactionSimple(?,?,?,?)}");
            sqlProc.registerOutParameter(1, Types.INTEGER );
            sqlProc.setString(2, strUserID );
            sqlProc.setString(3, strRetailerID );		
            sqlProc.setLong(4, amount );				
            sqlProc.execute( );
    ...
    class AddRetailerSimpleCmd extends Command implements Runnable {
        ...
        public StringBuffer execute(Connection connection,  Map<String, Object> mapUserData ) throws Exception {
            ...
            sqlProc = connection.prepareCall("{?=call addRetailerSimple(?)}");
            sqlProc.registerOutParameter(1, Types.INTEGER );
